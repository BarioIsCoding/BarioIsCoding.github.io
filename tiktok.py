...
def get_profile(session_id, device_id, iid):
    ...
    try:
        ...
        try:
            ...
            return "Error: Unexpected response format or content"
    except requests.exceptions.RequestException as e:
        return f"Network error occurred: {str(e)}"
    except Exception as e:
        return f"General error occurred: {str(e)}"
...
def change_username(session_id, device_id, iid, last_username, new_username):
    ...
    try:
        ...
        if "unique_id" in result and check_is_changed(last_username, session_id, device_id, iid):
            ...
            return f"Failed to change username. Response: {result}"
    except requests.exceptions.RequestException as e:
        return f"Network error occurred: {str(e)}"
    except Exception as e:
        return f"General error occurred: {str(e)}"
...
def main():
    parser = argparse.ArgumentParser(description="TikTok Username Changer")
    parser.add_argument("session_id", type=str, help="The session ID of the TikTok account")
    parser.add_argument("new_username", type=str, help="The new username you wish to set")
    ...
    user = get_profile(args.session_id, device_id, iid)
    if user != "None":
        print(f"Your current TikTok username is: {user}")
        print(change_username(args.session_id, device_id, iid, user, args.new_username))
    else:
        user_us = get_profile_us(args.session_id, device_id, iid)
        if user_us != "None":
            print(f"Your current TikTok username is: {user_us}")
            print(change_username_us(args.session_id, device_id, iid, user_us, args.new_username))
        else:
            print("Invalid session ID or other error.")
    print("discord.gg/conetic")
